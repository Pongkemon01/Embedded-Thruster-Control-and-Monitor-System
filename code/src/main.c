...
static uint8_t              au_pulse[ku_THUSTER_NUMBER][ku_DSHOT_COMMAND_SIZE];
static uint8_t              au_pulse_current[ku_THUSTER_NUMBER][ku_DSHOT_COMMAND_SIZE];
...
static void v_task_thruster( void *pv_parameters )
{
    ...
    for(;;)
    {
        ...
        memcpy( ( void * ) au_pulse_current, ( void * ) au_pulse, ku_THUSTER_NUMBER * ku_DSHOT_COMMAND_SIZE );
        ...
        if( HAL_TIM_PWM_Start_DMA( &x_tim3_handle, TIM_CHANNEL_3, ( uint32_t * )au_pulse_current[2U], ku_DSHOT_COMMAND_SIZE ) != HAL_OK )
        {
            v_error_handler();
        }
